...
// obj 给谁添加动画，  json 目标点, option 选项（time:时间，type：运动方式  complete:动画执行完成以后做什么）
function move(obj, json, option) {
    clearInterval(obj.timer)
    var option = option || {};
    option.type = option.type || Tween.Linear;
    option.step = option.step || 30; //步数
    obj.timer = null;
    ...
    obj.timer = setInterval(function () {
        ...
        for (var attr in json) {
            //算速度
            var cur = option.type(n,s[attr],json[attr] - s[attr],option.step);
            
//            console.log(json[attr] - s[attr]/option.step);
            // if (attr == 'opacity') {
            //     obj.style.opacity = cur;
            // } else {
            //     obj.style[attr] = cur + 'px'
            // }
            obj.style[attr] = cur + (attr=='opacity'?'':'px')
        }
